...
def move_to_paddle_x1(paddle_x, ball_pos, ball_vel, table_size, direction, move_factor):
...
def move_to_paddle(hitting_paddle_y, ball_x, ball_y, ball_dx, ball_dy, paddle_size, table_size, move_factor): # makes the ball actually hit the paddle
...
def do_hit1(paddle_x, paddle_y, ball_pos, ball_vel, paddle_size, ball_size, table_size, direction, move_factor, hit=False):
...
def do_hit(paddle_x, paddle_y, ball_x, ball_y, ball_dx, ball_dy, paddle_size, ball_size, table_size, direction, move_factor, hit=False):
    ...
    if ball_dx * -direction < 1:  # ball is not traveling (a) away from paddle (b) at a sufficient speed
        ...
        ball_dy = (ball_dy / abs(ball_dy)) * math.sqrt(ball_dx ** 2 + ball_dy ** 2 - 1)  # transform y velocity so as to maintain the speed
...
def hit_paddle1(paddle_x, paddle_y, ball_pos, ball_vel, paddle_size, ball_size, table_size, direction):
    ...
    while not 0 < ball_pos[0] < table_size[0]:
        ...
        if (paddle_x - (paddle_size[0] if direction < 0 else 0) < ball_pos[0] < paddle_x + (
                ...
                paddle_y - paddle_size[1] / 2 < ball_pos[1] < paddle_y + paddle_size[
                    1] / 2):
            #print("Hitting")
            ball_pos, ball_vel = do_hit1(paddle_x, paddle_y, ball_pos, ball_vel, paddle_size, ball_size,
...
def calc_hit(hitting_paddle_x, hitting_paddle_y, next_paddle_x, pre_hit_ball_x, pre_hit_ball_y, pre_hit_ball_dx, pre_hit_ball_dy, TABLE_SIZE, BALL_SIZE, PADDLE_SIZE, PADDLE_VEL, direction, move_factor):
...
def calc_hits(lower_bound, upper_bound, step, hitting_paddle_pos, next_paddle_pos, t_till_hit, pre_hit_ball_pos, pre_hit_ball_vel, TABLE_SIZE, BALL_SIZE, PADDLE_SIZE, PADDLE_VEL, direction, flag, done):
...
def calc_posibilities(hitting_paddle_pos, next_paddle_pos, t_till_hit, pre_hit_ball_pos, pre_hit_ball_vel, TABLE_SIZE, BALL_SIZE, PADDLE_SIZE, PADDLE_VEL, direction, flag, done):
    ...
    calc_thread = threading.Thread(target=calc_hits, args=(lower_bound, upper_bound, step, hitting_paddle_pos, next_paddle_pos, t_till_hit,
              pre_hit_ball_pos, pre_hit_ball_vel, TABLE_SIZE, BALL_SIZE, PADDLE_SIZE, PADDLE_VEL, direction, flag, done), daemon=True)
    calc_thread.start()
...
def pong_ai(paddle_frect, other_paddle_frect, ball_frect, table_size):
    ...
    if ball_vel[0] != 0 and ball_vel[1] != 0:
        if direction == side and calculating == 2:
            ...
            for hit in results:
                if lowest_reach < hit[0] < highest_reach:
                    ...
                    t_to_intercept = abs(hit[2] - other_paddle_pos[1]) // PADDLE_VEL
        ...
        if direction * side == -1 and calculating == -2:
            ...
            for hit in results:
                if lowest_reach < hit[0] < highest_reach:
                    t_to_me = t_till_hit + (hit[1] - other_paddle_pos[0]) // hit[3] + 30
                    t_to_intercept = abs(hit[2] - paddle_pos[1]) // PADDLE_VEL
                    weight = 1#sigmoid(t_to_intercept - t_to_me)
                    sum += hit[0] * weight
                    sum_of_weights += weight
            if sum_of_weights != 0 and t_till_hit > 50:
                goal_paddle_y = sum / sum_of_weights
            elif not prediction is None:
