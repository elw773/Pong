...
def calc_posibilities(hitting_paddle_pos, next_paddle_pos, t_till_hit, pre_hit_ball_pos, pre_hit_ball_vel, TABLE_SIZE,
    ...
    start = timer()
    calc_thread.start()
    print((timer()-start)/10e6)
    #threading.Thread(target=calc_hits, args=(lower_bound, upper_bound, step, hitting_paddle_pos, next_paddle_pos,
    #                                         t_till_hit,pre_hit_ball_pos, pre_hit_ball_vel, TABLE_SIZE, BALL_SIZE,
    #                                         PADDLE_SIZE, PADDLE_VEL, direction, flag, done),daemon=True).start()
...
def pong_ai(paddle_frect, other_paddle_frect, ball_frect, table_size):
            ...
            while i < len(results) and n < 20:
                ...
                if lowest_reach < hit[0] < highest_reach:
                    ...
                    if t_to_intercept > t_to_them:
                        ...
                        score = abs(hit[2] - TABLE_SIZE[1]/2)
                        #score *= hit[3]
                    if score > best_score:
    ...
    elapsed = (timer() - start) / 10e6
    if elapsed > 0.1:# and started_thread:
        print("pong_ai():", elapsed, started_thread)
